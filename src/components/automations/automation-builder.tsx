...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  send_message: { label: "Enviar Mensagem", icon: MessageSquare, border: "border-l-violet-500" },
  send_template: { label: "Enviar Modelo", icon: FileText, border: "border-l-violet-500" },
  add_tag: { label: "Adicionar Tag", icon: Tag, border: "border-l-violet-500" },
  remove_tag: { label: "Remover Tag", icon: TagIcon, border: "border-l-violet-500" },
  assign_conversation: { label: "Atribuir Conversa", icon: UserCheck, border: "border-l-violet-500" },
  update_contact_field: { label: "Atualizar Campo do Contato", icon: PencilLine, border: "border-l-violet-500" },
  create_deal: { label: "Criar Negócio", icon: Briefcase, border: "border-l-violet-500" },
  ...
  send_webhook: { label: "Enviar Webhook", icon: Webhook, border: "border-l-violet-500" },
  close_conversation: { label: "Fechar Conversa", icon: CircleSlash, border: "border-l-violet-500" },
}
...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; label: string; hint: string }[] = [
  { value: "new_message_received", label: "Nova Mensagem Recebida", hint: "Qualquer mensagem recebida" },
  {
    ...
    hint: "Primeira vez que este contato envia uma mensagem (funciona para contatos adicionados manualmente também)",
  },
  ...
  { value: "conversation_assigned", label: "Conversa Atribuída", hint: "Quando a conversa é atribuída a um atendente" },
...
function ConditionBranches({
  ...
  return (
    ...
    <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
      <BranchColumn label="Sim" color="text-violet-400">
...
function previewFor(step: BuilderStep): string {
  ...
  switch (step.step_type) {
    ...
    case "wait":
      const unit = (step.step_config.unit as string) ?? ""
      return `${step.step_config.amount ?? "?"} ${unitMap[unit] ?? unit}`
    case "condition":
      const sub = (step.step_config.subject as string) ?? ""
      return `quando ${subjectMap[sub] ?? sub}`
    case "send_webhook":
