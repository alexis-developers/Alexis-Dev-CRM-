...
export function ImportModal({ open, onOpenChange, onImported }: ImportModalProps) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error('Nenhuma linha válida encontrada. Certifique-se de que o CSV tenha um cabeçalho de coluna "phone".');
      setParsedRows([]);
      return;
    }
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (failed > 0) {
        toast.error(`Falha ao importar ${failed} contato${failed !== 1 ? 's' : ''}`);
      }
    } catch (err: unknown) {
          ...
          {preview.length > 0 && !result && (
            <div className="space-y-2">
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">
                Visualização (primeiras {preview.length} linhas)
              </p>
